# AUTO-GENERATED. Wraps detect_t9_*.sql as a BigQuery scheduled query
...
locals {
  query_template = file("${path.module}/../detect_t9.sql")
}

resource "google_bigquery_data_transfer_config" "detection" {
  display_name           = "ThreatModel detection: Bigquery.T9"
  location               = var.region
