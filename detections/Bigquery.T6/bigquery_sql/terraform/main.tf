# AUTO-GENERATED. Wraps detect_t6_*.sql as a BigQuery scheduled query
...
locals {
  query_template = file("${path.module}/../detect_t6.sql")
}

resource "google_bigquery_data_transfer_config" "detection" {
  display_name           = "ThreatModel detection: Bigquery.T6"
  location               = var.region
